...
import static com.atonementcrystals.dnr.vikari.lexer.LexerTestUtils.lex;
...
public class LexerTest_Comments {
    ...
    private void lexVariableDeclarationWithMultiLineComment(String sourceString, int expectedStatementCount,
                                                            CoordinatePair location1, CoordinatePair location2,
                                                            CoordinatePair location3, CoordinatePair location4,
                                                            CoordinatePair location5) {
        List<List<AtonementCrystal>> statements = lex(sourceString, expectedStatementCount);

        // TODO: This is a temporary design because crystals exist across multiple statements. Once
        //       BlankLineCrystals are removed, this will be simplified to expect a single statement.
        List<AtonementCrystal> crystals = statements.stream()
                .flatMap(List::stream)
                .filter(crystal -> !(crystal instanceof BlankLineCrystal))
                .toList();

        assertEquals(5, crystals.size(), "Unexpected number of crystals in statement.");

        testCrystal(crystals.get(0), ReferenceCrystal.class, "foo", location1);
        testCrystal(crystals.get(1), TypeLabelOperatorCrystal.class, ":", location2);
        testCrystal(crystals.get(2), TypeReferenceCrystal.class, "Integer", location3);
        testCrystal(crystals.get(3), LeftAssignmentOperatorCrystal.class, "<<", location4);
        testCrystal(crystals.get(4), IntegerCrystal.class, "2", location5);
    }
    ...
    public void testLexer_Crystals_MultiLineComment_TwoLineCommentBetweenTokensOfAssignmentStatement() {
        lexVariableDeclarationWithMultiLineComment("~:This is a\nmulti-line comment.:~ foo:Integer << 2",
                2, location(1, 22), location(1, 25), location(1, 26), location(1, 34), location(1, 37));

        lexVariableDeclarationWithMultiLineComment("foo~:This is a\nmulti-line comment.:~:Integer << 2",
                2, location(0, 0), location(1, 21), location(1, 22), location(1, 30), location(1, 33));

        lexVariableDeclarationWithMultiLineComment("foo:~:This is a\nmulti-line comment.:~Integer << 2",
                2, location(0, 0), location(0, 3), location(1, 21), location(1, 29), location(1, 32));

        lexVariableDeclarationWithMultiLineComment("foo:Integer ~:This is a\nmulti-line comment.:~ << 2",
                2, location(0, 0), location(0, 3), location(0, 4), location(1, 22), location(1, 25));

        lexVariableDeclarationWithMultiLineComment("foo:Integer << ~:This is a\nmulti-line comment.:~ 2",
                2, location(0, 0), location(0, 3), location(0, 4), location(0, 12), location(1, 22));

        lexVariableDeclarationWithMultiLineComment("foo:Integer << 2 ~:This is a\nmulti-line comment.:~",
                2, location(0, 0), location(0, 3), location(0, 4), location(0, 12), location(0, 15));
    }

    @Test
    @Order(3)
    public void testLexer_Crystals_MultiLineComment_ThreLineCommentBetweenTokensOfAssignmentStatement() {
        lexVariableDeclarationWithMultiLineComment("~:This is a\nthree-line\ncomment.:~ foo:Integer << 2",
                3, location(2, 11), location(2, 14), location(2, 15), location(2, 23), location(2, 26));

        lexVariableDeclarationWithMultiLineComment("foo~:This is a\nthree-line\ncomment.:~:Integer << 2",
                3, location(0, 0), location(2, 10), location(2, 11), location(2, 19), location(2, 22));

        lexVariableDeclarationWithMultiLineComment("foo:~:This is a\nthree-line\ncomment.:~Integer << 2",
                3, location(0, 0), location(0, 3), location(2, 10), location(2, 18), location(2, 21));

        lexVariableDeclarationWithMultiLineComment("foo:Integer ~:This is a\nthree-line\ncomment.:~ << 2",
                3, location(0, 0), location(0, 3), location(0, 4), location(2, 11), location(2, 14));

        lexVariableDeclarationWithMultiLineComment("foo:Integer << ~:This is a\nthree-line\ncomment.:~ 2",
                3, location(0, 0), location(0, 3), location(0, 4), location(0, 12), location(2, 11));

        lexVariableDeclarationWithMultiLineComment("foo:Integer << 2 ~:This is a\nthree-line\ncomment.:~",
                3, location(0, 0), location(0, 3), location(0, 4), location(0, 12), location(0, 15));
    }

    // TODO: Add more comment tests once BlankLineCrystals are removed from the Lexer.
}
